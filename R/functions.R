...
scrape_next_cards <- function(link){
  
  link %>% 
    read_html() %>% 
    html_nodes(".b-link_style_white") %>% 
    html_attr("href") %>% 
    tibble("cards" = .)
}
...
scrape_fight_summary_data <- function(link){
  ...
  summary_data <- table_df[1] %>% 
    ...
    separate(fighter_2_TD, into = c("fighter_2_TD_Landed", "fighter_2_TD_Attempts"), 
             sep = " of ", extra = "merge") 
   # mutate_at(vars(contains("Percent")), ~.01*str_replace(.x, "%", ""))  
